...
public class UserController extends BaseController {
    ...
    public String login(User user, HttpServletRequest request) {
        ...
        try {
            user.setUserpassword(DigestUtils.md5DigestAsHex(password.getBytes("utf-8")));
    ...
    public String updatepwd(int id, String userpassword, HttpServletRequest request) {
        ...
        if (oldpassword.equals(user.getUserpassword())) {
            try {
                userDAO.updatepwd(id, DigestUtils.md5DigestAsHex(userpassword.getBytes("utf-8")));
            } catch (UnsupportedEncodingException e) {
    ...
    public String selectAll(HttpServletRequest request) {
        ...
        distributorMap.put("key", key);
        distributorMap.put("usertype", "配送员");
        //获取配送员列表
        System.out.println("配送员列表");
        List<User> distributorList = userDAO.selectAll(distributorMap);
        for (User user : distributorList) {
            System.out.println(user);
            userList.add(user);
        }

        // 将配送员列表 加到 员工列表中
        System.out.println("合到一起之后");
        for (User user : userList) {
            System.out.println(" user = " + user);
        }
