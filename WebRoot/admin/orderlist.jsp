                ...
                <div class="layui-inline">
                    <div class="layui-input-inline">
                        <form action="<%=path %>/admin/orderList.do" method="post" id="searchform" name="searchform">
                            <input value="${key}" placeholder="请输入订单编号" name="key"
                ...
                <table class="layui-table" style="font-size:12px;">
                    ...
                    <c:forEach items="${list}" var="order" varStatus="status">
                        <tr>
                            ...
                            <td><font style="font-weight: bold;">地址:&nbsp;&nbsp;</font>${order.address.addr}&nbsp;&nbsp;&nbsp;&nbsp;<font
                                    style="font-weight: bold;">${order.address.name}&nbsp;&nbsp;&nbsp;&nbsp;${order.address.tel}</font>
                            </td>
                            <td>${order.shfs}</td>
                            ...
                            <td>
                                <c:if test="${order.shstatus eq '待发货'}">
                                    <a class="layui-btn layui-btn-mini" href="faHuo.do?id=${order.id}"><i
                                            class="iconfont icon-edit"></i> 发货</a>
                                </c:if>
                                <c:if test="${order.shfs eq '送货上门' and order.shstatus eq ''}">
                                    <a class="layui-btn layui-btn-mini" href="faHuo.do?id=${order.id}"><i
                                            class="iconfont icon-edit"></i> 发货</a>
                                </c:if>
                            </td>
                            <td>
                                <c:if test="${order.shstatus eq '运送中' }">
                                    <%--                                    <a class="layui-btn layui-btn-mini" href="faHuo.do?id=${order.id}"><i--%>
                                    <%--                                            class="iconfont icon-edit"></i> 发货</a>--%>
                                    <p>
                                        配送人员姓名:${deliveryman.username}
                                        联系方式：${deliveryman.tel}
                                    </p>
                                </c:if>
                    ...
                    <tr>
                        <td align="center" style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue"
                            colspan="10">
                            共${total}条记录&nbsp;&nbsp;&nbsp;&nbsp;
                            <a href="orderList.do?index=1&key=${key}" style="font-family:楷体;">首页</a>&nbsp;&nbsp;

                            <c:choose>
                                <c:when test="${index >1}">
                                    <a href="orderList.do?index=${index-1}&key=${key}"
                                       style="font-family:楷体;">上一页</a>
                                </c:when>
                                <c:otherwise>
                            ...
                            &nbsp;&nbsp;
                            <a href="orderList.do?index=${pages}&key=${key}" style="font-family:楷体;">末页</a>
                        </td>
                    </tr>
                    </tbody>
                </table>
