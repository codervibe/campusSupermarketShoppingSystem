					...
					<tbody class="news_content" id="userTbody">
					   <c:forEach items="${list}" var="member" varStatus="status">
						<tr >
							...
							<td>${member.status}</td>
							<td>
							<c:if test="${member.status eq '在用'}">
							<a class="layui-btn layui-btn-mini" href="changeStatus.do?id=${member.id}&type=sd">锁定</a>
							</c:if>
							<c:if test="${member.status eq '已禁用'}">
							<a class="layui-btn layui-btn-mini" href="changeStatus.do?id=${member.id}&type=js">解锁</a>
							</c:if>
							</td>
						</tr>
						</c:forEach>
