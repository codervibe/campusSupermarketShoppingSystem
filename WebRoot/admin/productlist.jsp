					   ...
					   <c:forEach items="${list}" var="product" varStatus="status">
						<tr >
							...
							<c:if test="${product.tprice gt 0}">
							  ...
							  <a href="tpriceadd.jsp?id=${product.id}&price=${product.price}">特价</a>
							</td>
						...
						<tr>
                    <td align="center" style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue" colspan="12">
                                                                 共${total}条记录&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="productList.do?index=1&key=${key}&key1=${key1}" style="font-family:楷体;">首页</a>&nbsp;&nbsp;
                        
                        <c:choose>
                        <c:when test="${index >1}">
                        <a href="productList.do?index=${index-1}&key=${key}&key1=${key1}" style="font-family:楷体;">上一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        <a href="productList.do?index=${index+1}&key=${key}&key1=${key1}" style="font-family:楷体;">下一页</a>
                        </c:when>
                        <c:otherwise>
                        ...
                        &nbsp;&nbsp;
                        <a href="productList.do?index=${pages}&key=${key}&key1=${key1}" style="font-family:楷体;">末页</a>
                    </td>
                  </tr>
					</tbody>
					</table>
...
function updateFkstatus(id){
	...
	layer.confirm('商品是否付款？', {
    	  btn: ['是','否'] //按钮
    		...
    		location.replace("/onlineshopssm/admin/updateFkstatus.do?id="+id);
